"""
Takes the official SIT Nagpur College Template:
'CA3_Mini Project Report Template _Flexi credit course Ahentic AI & Automation.docx'
and populates it with complete academic content for Salim Ansari (PRN: 24070521005),
guided by Dr. Parag Naik and Dr. Shreyas Rajendra Hole.
"""
...
def generate_full_report_from_template():
    tpl_path = r"C:\Users\Salim Ansari\Downloads\CA3_Mini Project Report Template _Flexi credit course Ahentic AI & Automation.docx"
    doc = docx.Document(tpl_path)

    title_text = "“Automated Literature Review Assistant: An Agentic AI-Powered Autonomous Academic Research Discovery, Gap Intelligence, and Review Synthesis System”"
    ...
    # 1. Update Title Page / Cover Page
    for i, p in enumerate(doc.paragraphs[:35]):
        txt = p.text
        if "<PROJECT TITLE NAME>" in txt or "PROJECT TITLE" in txt:
            p.text = ""
            ...
            r.font.color.rgb = RGBColor(0, 0, 0)
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        elif "Dr./Prof. <XYZ>" in txt or "<Guide Name>" in txt:
            p.text = ""
    ...
    # 2. Certificate Page
    for i, p in enumerate(doc.paragraphs[30:40]):
        if "<Title of the Project>" in p.text or "This is to certify that" in p.text:
            p.text = ""
            r = p.add_run(
    ...
    for i, p in enumerate(doc.paragraphs[38:51]):
        if "<Project title>" in p.text or "I hereby declare that" in p.text:
            p.text = ""
            r = p.add_run(
            ...
            p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            p.paragraph_format.line_spacing = 1.25
        elif "Name of Student <1>:" in p.text or "<XYZ (PRN)>" in p.text:
            p.text = f"Name of Student: {student_name} (PRN: {prn})"
            p.style.font.name = "Times New Roman"
            p.style.font.size = Pt(11)
        elif "< Title of the project >" in p.text or "Title of the project:" in p.text:
            p.text = f"Title of the project: {short_title}"
            p.style.font.name = "Times New Roman"
            p.style.font.size = Pt(11)
        elif "<Student1 Name and Signature>" in p.text:
            p.text = f"Signature: ______________________ ({student_name})"
            p.style.font.name = "Times New Roman"
            p.style.font.size = Pt(11)

    # 4. IPR Declaration
    for i, p in enumerate(doc.paragraphs[50:67]):
        if "entitled TITLE" in p.text or "entitled 'TITLE'" in p.text or "entitled <TITLE>" in p.text or "We hereby declare that the project entitled" in p.text:
            p.text = ""
            r = p.add_run(
            ...
            p.text = f"Student Name: {student_name} (PRN: {prn})"
            p.style.font.name = "Times New Roman"
            p.style.font.size = Pt(11)

    # 5. Abstract & Keywords
    for i, p in enumerate(doc.paragraphs[80:90]):
        if "This electronic document is a" in p.text or "ABSTRACT" in p.text:
            if "ABSTRACT" not in p.text:
                p.text = ""
                r = p.add_run(
            ...
            p.text = ""
            r_k = p.add_run("Keywords: ")
            r_k.font.name = "Times New Roman"
            r_k.font.size = Pt(10)
            r_k.font.bold = True
            r_k.font.color.rgb = RGBColor(0, 0, 0)
            r_v = p.add_run("Agentic AI, Literature Review Automation, Research Gap Intelligence, Retrieval-Augmented Generation (RAG), FAISS, Semantic Scholar, ArXiv API, Voice-Driven Multimodal Assistant, Symbiosis Institute of Technology.")
            r_v.font.name = "Times New Roman"
            r_v.font.size = Pt(10)
            r_v.font.italic = True
            r_v.font.color.rgb = RGBColor(0, 0, 0)

    # 6. Delete template placeholder paragraphs from Chapter 1 onwards and replace with full content
    # Find paragraph index where Chapter 1 begins
    chap1_idx = None
    for i, p in enumerate(doc.paragraphs):
        if "Chapter 1" in p.text or "CHAPTER 1" in p.text:
            if i > 100: # After TOC
                chap1_idx = i
                break
    
    if chap1_idx is not None:
        # Delete old placeholder paragraphs from chap1_idx onwards
        for _ in range(len(doc.paragraphs) - chap1_idx):
            p = doc.paragraphs[chap1_idx]
            p._element.getparent().remove(p._element)

    # Helper functions to add stylized academic elements
    ...
    print("[SUCCESS] Report generation from college template complete!")

if __name__ == "__main__":
    generate_full_report_from_template()
